...
pytest --cov=rss_watcher         # with coverage report
...
Tests use `pytest-asyncio` with `asyncio_mode = "auto"` (configured in `pyproject.toml`). Shared fixtures are in `tests/conftest.py`, and sample XML feeds are in `tests/fixtures/`.
