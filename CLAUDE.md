...
```text
config.yaml → load_config() → RSSWatcher.start()
                                    ↓
        ┌───────────────────────────┴───────────────────────────┐
        ↓                           ↓                           ↓
...
- **HTML entity handling:** RSS content HTML entities are resolved while preserving XML entities during parsing to prevent double-encoding issues in displayed content.
...
# Run with coverage
pytest --cov=rss_watcher --cov-report=html

# Run specific test file
pytest tests/test_filters.py -v

# Run tests matching pattern
pytest -k "test_keyword_filter"
```

**Test structure:** Each module has a corresponding test file in `tests/`. Tests use `aioresponses` for mocking HTTP requests and `pytest-asyncio` for async fixtures. The `conftest.py` file contains shared fixtures.
